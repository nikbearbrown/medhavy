...
[FIGURE: The data production architecture. Two columns side by side: "Interaction Design" and "Evidence Generated." Hint-before-answer → scaffolding response curve. Spaced retrieval → retrieval strength decay signature. Confidence before correctness → uncertainty calibration. Temporal session logs → engagement pattern. Sequential error logging → trajectory coherence. Cross-context application prompts → transfer evidence. Social knowledge texture logs → genuine encounter evidence. Caption: Every Medhavy interaction is designed to produce learning and evidence simultaneously. Neither is a byproduct of the other.]
...
[FIGURE: Three constraints → one conclusion. Three boxes labeled "Live instrumentation required," "Authentic context evidence is richest," and "Measurement requires specific interaction design" — all three arrows pointing to a single box labeled "The measurement infrastructure and the learning infrastructure are the same system." Caption: The platform requirement isn't a design preference. It follows from three independent constraints that all lead to the same place.]
