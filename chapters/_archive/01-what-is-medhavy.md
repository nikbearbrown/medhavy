...
[FIGURE: Tic TOC session flow — interview → concept node map → three outputs (book, Medhavy course, Canvas course). Caption: The professor's pedagogical intent, made explicit and structured in a two-hour session, becomes the concept map Medhavy uses to track where each student is and what they're ready for.]
...
[FIGURE: The measurement hierarchy — behavioral events (PostHog) → GLP components → reward signal → bandit update → mode selection. Caption: What gets measured shapes what gets optimized. The GLP framework exists to ensure the reward signal tracks genuine learning rather than engagement proxies.]
